...
export class WorkorderService {
  ...
  private dataWorkordersJSON: Array<WorkorderJSON>;
  ...
  private getWorkorderByJSON(the_url: string) {
    this.httpClient.get<Array<WorkorderJSON>>(the_url).subscribe(
      (data) => {
        console.log(data);
        let mappedData = data.map((d) => ({Id: +d.orderData.header.orderNumber, Title: d.return.message}));
  ...
  public getMappedJSONWorkorders(): Observable<Workorder[]> {
    ...
    return this.httpClient
      ...
      .pipe(
        map( (data) => {
          // let result = [{Id: 321, Title: 'Three Two One'}];
          console.log('getMappedJSONWorkorders map');
          console.log(data);
          return data.map(
            (d) => ({Id: +d.orderData.header.orderNumber, Title: d.return.message})
          );
        }
        )
      )
